...
public class FormantDetector extends Object
{
	...
	//  CONSTRUCTOR
	//--------------------------------------
	public function FormantDetector( inSpectrum:SpectralAnalysis, inPitch:Operator, inMethod:String ) {
		...
		if( _method != DetectionType.FORMANTS_ORIGINAL_METHOD ) {
			_spectrumBlurY = _spectrum.asBitmapData();
			_spectrumBlurXY = _spectrumBlurY.clone();
		
			var blurWidth:Number = DetectionType.smoothness( _method );
		...
		if( _method == DetectionType.FORMANTS_ORIGINAL_METHOD ) {
			...
			*/
				...
				if( false ) {
					...
					vuRatio = Num.clamp( (maxEnergyRatio-Const.UNVOICED_ENERGY_RATIO) / (Const.VOICED_ENERGY_RATIO-Const.UNVOICED_ENERGY_RATIO), 0.0, 1.0 );	// 0..1
					VURatios[i] = vuRatio;
			...
			originalPickAndStoreFormants( Const.VOICED_OPS, VPowers, VFreqs, UPowers, UFreqs, _spectrum.freqs, VURatios );
			...
			for( i=0; i<frame.length; i++ ) {
				...
				power = Number( _spectrumBlurXY.getPixel( _index, i )) * (1.0/255);
	...
	//--------------------------------------
	private function originalPickAndStoreFormants( count:int, VPowers:Vector.<Number>, VFreqs:Vector.<Number>, UPowers:Vector.<Number>, UFreqs:Vector.<Number>, bandFreqs:Vector.<Number>, vuRatios:Vector.<Number> ) :void {
		var v:int, i:int;
